...
os.environ["OAUTHLIB_INSECURE_TRANSPORT"] = "1"  # for http callback error
app = Flask(__name__)
app.secret_key = "abcd"
...
slack_redirect_uri = "https://6e34-49-207-246-70.ngrok-free.app/slack_callback"
...
def slack_callback():
    slack_channels = []
    try:
        # scope = ["channels:read", "channels:history", "chat:write", "groups:read", "mpim:read", "im:read", "users:read"]
        slack_session = OAuth2Session(slack_client_id, redirect_uri=slack_redirect_uri)
        ...
        print("Slack Channels: {}".format(get_channels(token)))
        slack_channels = get_channels(token)
    except Exception as e:
        print('error >>>', e)
        return 'Error: {}'.format(e)
    return f"Slack Channels: {', '.join(slack_channels)}"

def get_channels(token_json):
    req = requests.get(
    ...
    res = req.json()
    return [channel['name'] for channel in res['channels']]
...
def login():
    ...
    print("login api called...", authorization_url)
...
def callback():
    ...
    return "Token: {}<p />Projects: {}".format(
        token_json, ", ".join(get_projects(token_json))
    )
...
def get_projects(token_json):
    ...
    jira = Jira(
        url="https://api.atlassian.com/ex/jira/{}".format(cloud_id), oauth2=oauth2_dict
    )
